...
export default function LabPage() {
  ...
  const handleStart = () => {
    ...
    playTrack('hero')
    setScene('loading')
    // Apply low-pass filter for loading transition
  ...
  const handleModuleClick = (moduleId: string) => {
    setScene(`module-${moduleId}` as Scene)
  }
  ...
  return (
    <main className="h-screen w-screen overflow-hidden bg-black relative">
      <AnimatePresence mode="wait">
        ...
        {scene === 'modules' && (
          <ModulesSection key="modules" onNext={() => setScene('audience')} onModuleClick={handleModuleClick} />
        )}
