...
export default function TerminalLoader({ onComplete }: TerminalLoaderProps) {
    ...
    return (
        ...
        >
            ...
            <div
                ref={containerRef}
                className="relative z-[10] w-[90vw] max-w-[700px] p-6 tablet:p-8 pc:p-8"
            >
                {/* Terminal header */}
                <div className="flex items-center gap-2 mb-6 pb-3 border-b border-neon-green/20">
                    <div className="flex gap-1.5 shrink-0">
                        <div className="w-2 h-2 tablet:w-2.5 tablet:h-2.5 bg-red-500/80 rounded-full" />
                        <div className="w-2 h-2 tablet:w-2.5 tablet:h-2.5 bg-yellow-500/80 rounded-full" />
                        <div className="w-2 h-2 tablet:w-2.5 tablet:h-2.5 bg-neon-green/80 rounded-full" />
                    </div>
                    <span className="ml-2 tablet:ml-4 font-sans text-[8px] tablet:text-[10px] text-neon-green/50 uppercase tracking-[0.2em] tablet:tracking-[0.3em] truncate">
                        PROTOCOLO_SAUCE://SYSTEM_CORE
                    </span>
                </div>

                {/* Terminal lines */}
                <div className="space-y-2 mb-8 min-h-[200px] tablet:min-h-[240px] pc:min-h-[240px]">
                    {terminalLines.slice(0, visibleLines).map((line, i) => (
                        <motion.div
                            key={i}
                            className="font-mono text-xs tablet:text-sm pc:text-sm overflow-hidden whitespace-nowrap"
